...
public class FadedTreeVar1Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		if (!world.isClientSide()) {
			customSappling = DreamscapeModBlocks.FADED_SAPLING.get().defaultBlockState();
			airTag = "dreamscape:saplingplaceables";
			structureOffsetX = 0;
			structureOffsetY = 0;
			structureOffsetZ = 0;
			conditionOffsetX = 0;
			...
			if (!(airTag).isEmpty()) {
				...
				if (!dontSpawn && rotation == 0) {
					if (world instanceof ServerLevel _serverworld) {
						StructureTemplate template = _serverworld.getStructureManager().getOrCreate(new ResourceLocation("dreamscape", "spiral_oak_tree_1"));
						if (template != null) {
